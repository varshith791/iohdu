  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      if (data.token) {
        ...
        await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/attendance/login`, {
          method: 'POST',
    ...
    <main className="min-h-screen bg-background text-foreground flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md space-y-8 animate-in slide-in-from-bottom-4 duration-700 relative z-10">
        ...
        <div className="bg-card border border-border shadow-2xl rounded-[2.5rem] p-10 space-y-8">
            ...
            {!isLogin && (
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  className="w-full bg-secondary border border-transparent rounded-2xl p-4 outline-none focus:border-primary/30 focus:ring-4 focus:ring-primary/5 transition-all font-medium"
          ...
          <button 
            onClick={handleAuth}
            className="w-full bg-primary text-primary-foreground rounded-2xl py-4 font-bold shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all"
          >
            {isLogin ? 'Sign In to Console' : 'Register Admin'}
          </button>

          <div className="space-y-4">
            <p className="text-center text-sm text-muted-foreground font-medium">
              {isLogin ? "Need an account? " : "Already an admin? "}
              <button 
            ...
            <button onClick={() => router.push('/')} className="w-full text-xs text-muted-foreground/60 font-bold hover:text-primary transition-all pt-4 border-t border-border/50 uppercase tracking-widest">
              ← Back to Platform
            </button>
          </div>
        </div>
